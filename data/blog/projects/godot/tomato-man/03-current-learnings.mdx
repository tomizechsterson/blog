...
# This is within physics_process() in our state node, among other things
...
there's already a [GitHub Action](https://github.com/marketplace/actions/godot-ci) and container available that were made
for exporting Godot projects.
...
on a branch, and simply configure the repo to initiate a build when a commit goes to that branch. Oh well, lesson learned.
...
a hassle, so I rage-quit enums, found an article (or was given a link to one) that showed how it's done in Godot, and just went
for it.
...
entities. I do remember that the player object wasn't even setup with an enum variable for state while those first four
were being implemented, so maybe that made it more difficult than it normally would be.

Either way, that should about sum up the major learnings so far. Now, it's time to actually, finally get back into this
project, so I can continue learning how to make stuff in Godot, and secondarily, blog with some kind of purpose.
