...
What we do want is to display some stats to the user, and ideally some way they can challenge themself, if they so desire.
The idea is to show how many coins were collected, and how many enemies were defeated (hopefully, I get this far; enemies
are a few steps down the line). What would be _really_ nice is to implement a level timer that records how long it took to
get to the end. This way, the user could try to beat their best time, or we could even implement a scoreboard. For the first
iteration, we'll stick to only displaying the stats and best time.
...
straightforward, and there are less signals in general being thrown around. On the other hand, chaining signals up the tree
sounds like it could get confusing and messy pretty quick. Although, if nodes on the path also needed to respond to the
signal, this would actually be pretty nice.

After some research,[^1] it looks like it's generally considered good practice to chain signals up the tree. It's said that
...
[^1]: Which was really just reading the AI response and a single Reddit thread to a single search query

[^2]:
    Yes, it's normally Now, Next, Later, but I like alliteration. This is really just what's currently on my mind to do
    after the stuff under Next. It will also probably change fairly often.
