...
[//]: # 'Also add note about age, and being more cognizant about focus and learning?'
...
collisions between it and Tomato Man, and when a collision is detected, the new damage state is working nicely. Just
need a short invulnerability time, and then we can move to player death state, and then probably attack. Once these other
two are done, I'm thinking that will justify another release to GameJolt, so that'll be good.
...
it is infected with typescript). Until next time!
