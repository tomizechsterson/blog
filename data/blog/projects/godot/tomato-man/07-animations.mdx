...
minute or two, so you'll just have to imagine it while replacing the word "developers" with "animations")
...
Back in the day, when I was a young lad in 2023[^2] I was doing a lot of tutorials. Now, when I say "a lot of tutorials", what
I'm really saying is "WAY too many tutorials. As in, probably 10x more than I should have. As a matter of fact, I carry
great shame for the innumerable, obnoxious, unspeakable amount of tutorials that I did back then." One thing that I saw in
...
`AnimationTree` node.[^3] Much like what's pictured below:
...
exactly the same way. The real difference here is that instead of telling a state mechine to travel to states, you set a
'transition request' parameter and then set the blend position like before.
...
output node. Any number of 'global' states can be added in this way, and they'll all be applied regardless of what state is
selected in the `AnimationNodeTransition` (named "Transition" in the image above). Simply set their `add_amount` value like so:
